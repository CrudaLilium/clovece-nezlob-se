...
import { ETurnState, Game } from './Game/Game';
import { IBoardCell, IPlayer, IPlayerCell } from './Game/GameStructs';

export interface IGameBoardProps {
    game: Game;
}

export class GameBoard extends React.Component<IGameBoardProps, { lastRoll?: number }>
{
    ...
    private drawBoard() {
        ...
        const boardInfo = this.props.game.board;
        const playerCellsInfo = this.props.game.playerCells;

        for (let cell of [...boardInfo, ...playerCellsInfo]) {
            ctx.fillStyle = ('player' in cell && (cell as { player: IPlayer }).player.color) || "#EBEBEB";
    ...
    private drawHitDetection() {
        ...
        const playerCells = this.props.game.playerCells;
    ...
    private onHit(event: React.MouseEvent<HTMLCanvasElement, MouseEvent>) {
        ...
        const currTurn = this.props.game.turn.currentPlayer;
        this.props.game.pieceSelect(pixel[0]);
        this.drawPlayers();
        this.drawHitDetection();
        //TODO: refactor to event driven, or make "turn" immutable and assign
        if (this.props.game.turn.currentPlayer !== currTurn)
            this.setState({ lastRoll: 0 });
    }

    private roll() {
        const lastRoll = this.props.game.roll();
        this.setState({ lastRoll: lastRoll });
        this.drawPlayers();
    }

    public render(): JSX.Element {
        const turnInfo = this.props.game.turn;
        return (
            <div style={{ display: 'flex' }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", marginLeft: "1rem", width: 250 }}>
                    ...
                    <div>
                        <button onClick={this.roll} className={"big-button"}>Roll dice!</button>
